...
public abstract class Entity extends AbstractNode implements IPropertyChangeListener, Iconified {
    ...
    public void delete(INode child) {
        Entity childEntity = (Entity) child;
        // Не проверяю "ID == null" для того чтобы при обновлении плагина сбрасывался кэшированный PackageView
        // (т.к. он хранит ссылку на файл JAR). А при установке нового пакета создается новый PackageView и
        // кэшируется
        if (Entity.getDefinition(childEntity.getClass()).autoGenerated() /*&& childEntity.getID() == null*/) {
...
//    public void delete(INode child) {
//        Entity childEntity = (Entity) child;
//        if (Entity.getDefinition(childEntity.getClass()).autoGenerated() || !allowModifyChild()) {
