...
public interface ICommand<V, C> extends Iconified {
    ...
    /**
     * Актуализация состояния доступности команды. Поскольку заранее неизвестно от чего может зависеть состояние
     * доступности команды, данный метод вызывается при изменении либо сотава контекста, либо внутреннего состояния
     * элементов контекста.
     */
    public void activate();
    
    /**
     * Установка контекста исполнения команды.
